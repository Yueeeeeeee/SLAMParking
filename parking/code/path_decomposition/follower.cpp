...
void globalGoalPositionCallback(const geometry_msgs::PoseStamped::ConstPtr& msg);
void nextGoalPositionCallback(const geometry_msgs::PoseStamped::ConstPtr& msg);
...
{   
    ...
    ros::Subscriber goalPositionSubscriber = n.subscribe("move_base_simple/goal", INPUT_BUFFER_SIZE, globalGoalPositionCallback);
    ros::Subscriber nextGoalSubscriber = n.subscribe("nextgoal", INPUT_BUFFER_SIZE, nextGoalPositionCallback);
    ...
    n.param<double>("tvc/gains/P", tvcGainP, 2.4);
    n.param<double>("tvc/gains/I", tvcGainI, 9999.0);
    n.param<double>("tvc/gains/D", tvcGainD, 0.5);
...
void actualPositionCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg) {
    ...
    /* Usual code. Commented for a simple filter test
    actualValues.positionX = msg->pose.pose.position.x;
    actualValues.positionY = msg->pose.pose.position.y;
    */
    // FIXME: FIR filter just for testing! Use Kalman filter later!
    actualValues.positionX = 0.5 * msg->pose.pose.position.x + 0.5 * actualValues.positionX;
    actualValues.positionY = 0.5 * msg->pose.pose.position.y + 0.5 * actualValues.positionY;
 ...
 * This function determines if a goal exists.
 */
void globalGoalPositionCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    // Goal from Rviz.
    goalExists  = true;
...
/**
 * ROS callback function for the '/nextgoal' topic.
 * This function sets the controller goal position.
 */
void nextGoalPositionCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    // Set next point to drive to
    if(goalExists == true) {
