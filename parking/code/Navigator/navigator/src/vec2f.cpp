...
double Vec2f::angle(Vec2f& other) {
    Vec2f thisNorm = this->normalize();
    Vec2f thatNorm = other.normalize();
	double rawAngle = acos(thisNorm.dot(thatNorm));
	double side = thisNorm.x * thatNorm.y - thisNorm.y * thatNorm.x;
	if(side >= 0.0) {
		return rawAngle;
	}
	else{
